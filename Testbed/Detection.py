...
class FlowAnalyzer(app_manager.RyuApp):
    ...
    def flow_stats_reply_handler(self, ev):
        ...
        for eth_src, stats in self.flow_stats.items():
            packet_arrival_frequency = self.calculate_packet_arrival_frequency(stats)
            information_gain = self.calculate_information_gain(stats)
            spoofed_ip_detected = self.check_spoofed_ip(stats)
            mean_duration, cv_duration = self.calculate_mean_and_cv([s.duration_sec for s in stats])
            mean_packets, cv_packets = self.calculate_mean_and_cv([s.packet_count for s in stats])
            mean_bytes, cv_bytes = self.calculate_mean_and_cv([s.byte_count for s in stats])
    ...
    def calculate_information_gain(self, stats):
